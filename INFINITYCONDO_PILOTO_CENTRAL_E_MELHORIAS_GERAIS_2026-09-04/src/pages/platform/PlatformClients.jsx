...
function PlatformClients() {
  ...
  async function load() {
    ...
    try {
      ...
      const [result, planItems] = await Promise.all([
        platformApi.condominiums.clients(`?${params.toString()}`),
        platformApi.plans.list(),
      ]);
      setData(result ?? { items: [], summary: {} });
      setPlans(Array.isArray(planItems) ? planItems.filter((item) => item.active !== false) : []);
    } catch (err) {
  ...
  async function changePlan() {
    if (!selected?.id || !selectedPlanId) return;
    setActionBusy(true); setError("");
    try {
      await platformApi.condominiums.changePlan(selected.id, { planId: selectedPlanId });
      setSelected(null); setSelectedPlanId(""); await load();
    } catch (err) { setError(err?.message ?? "Não foi possível alterar o plano."); }
    finally { setActionBusy(false); }
  }

  async function executeLifecycle() {
    if (!selected?.id || !confirmAction?.action) return;
    setActionBusy(true); setError("");
    try {
      await platformApi.condominiums.lifecycle(selected.id, { action: confirmAction.action, reason: confirmAction.reason ?? "" });
      setConfirmAction(null); setSelected(null); await load();
    } catch (err) { setError(err?.message ?? "Não foi possível alterar o status do cliente."); }
    finally { setActionBusy(false); }
  }
  ...
  return (
    <div style={styles.page}>
              ...
              <tbody>
                {items.map((client) => (
                  <tr key={client.id} style={styles.row}>
                    ...
                    <td style={styles.td}>
                      <button type="button" style={styles.detailsButton} onClick={() => { setSelected(client); setSelectedPlanId(client.subscription?.plan?.id ?? ""); }}>Ver ficha</button>
                    </td>
                  </tr>
                ))}
              </tbody>
        ...
        <div style={styles.overlay} onMouseDown={(event) => event.target === event.currentTarget && setSelected(null)}>
          <div style={styles.modal}>
            ...
            <section style={styles.manageCard}>
              <div>
                <span style={styles.heroEyebrow}>GESTÃO DO CONTRATO</span>
                <h3 style={{ margin: "6px 0 4px", color: "#2e174f" }}>Plano e ciclo do cliente</h3>
                <p style={{ margin: 0, color: "#6b7280", fontSize: 13 }}>Upgrade e downgrade preservam todos os dados. Recursos premium apenas ficam bloqueados quando o plano não os inclui.</p>
              </div>
              <div style={styles.manageActions}>
                <select style={styles.select} value={selectedPlanId} onChange={(event) => setSelectedPlanId(event.target.value)}>
                  <option value="">Selecione o plano</option>
                  {plans.map((plan) => <option key={plan.id} value={plan.id}>{plan.name} • {money(plan.monthlyPriceInCents)}</option>)}
                </select>
                <button type="button" style={styles.actionPrimary} disabled={actionBusy || !selectedPlanId || selectedPlanId === selected.subscription?.plan?.id} onClick={changePlan}>Alterar plano</button>
                {selected.status === "SUSPENDED" ? (
                  <button type="button" style={styles.actionSuccess} onClick={() => setConfirmAction({ action: "REACTIVATE", title: "Reativar cliente", text: "O acesso do condomínio e a assinatura voltarão ao estado ativo." })}>Reativar</button>
                ) : selected.status !== "CANCELED" ? (
                  <button type="button" style={styles.actionWarn} onClick={() => setConfirmAction({ action: "SUSPEND", title: "Suspender cliente", text: "Os usuários do condomínio terão o acesso operacional bloqueado até a reativação." })}>Suspender</button>
                ) : null}
                {selected.status !== "CANCELED" && <button type="button" style={styles.actionDanger} onClick={() => setConfirmAction({ action: "CANCEL", title: "Cancelar assinatura", text: "O cliente será marcado como cancelado. Os dados permanecerão preservados para histórico." })}>Cancelar assinatura</button>}
              </div>
            </section>
      ...
      {confirmAction && (
        <div style={styles.overlay} onMouseDown={(event) => event.target === event.currentTarget && setConfirmAction(null)}>
          <div style={{ ...styles.modal, maxWidth: 520 }}>
            <div style={styles.modalHeader}>
              <div><span style={styles.heroEyebrow}>CONFIRMAÇÃO</span><h2 style={styles.modalTitle}>{confirmAction.title}</h2></div>
              <button type="button" style={styles.close} onClick={() => setConfirmAction(null)}><FaTimes /></button>
            </div>
            <p style={{ color: "#6b7280", lineHeight: 1.65 }}>{confirmAction.text}</p>
            {confirmAction.action === "CANCEL" && <textarea style={{ ...styles.search, minHeight: 90, paddingLeft: 14 }} placeholder="Motivo do cancelamento (opcional)" value={confirmAction.reason ?? ""} onChange={(event) => setConfirmAction((current) => ({ ...current, reason: event.target.value }))} />}
            <div style={{ ...styles.modalFooter, justifyContent: "flex-end" }}>
              <button type="button" style={styles.detailsButton} onClick={() => setConfirmAction(null)}>Voltar</button>
              <button type="button" style={confirmAction.action === "CANCEL" ? styles.actionDanger : styles.actionPrimary} disabled={actionBusy} onClick={executeLifecycle}>{actionBusy ? "Processando..." : "Confirmar"}</button>
            </div>
          </div>
        </div>
      )}
...
const styles = {
  ...
  hero: { display: "flex", alignItems: "center", justifyContent: "space-between", gap: "24px", padding: "28px", borderRadius: "26px", background: "linear-gradient(135deg,#241047,#5b21b6 55%,#8b5cf6)", color: "white", boxShadow: "0 24px 60px rgba(91,33,182,.22)" },
  ...
  metric: { display: "flex", alignItems: "center", gap: "14px", padding: "18px", borderRadius: "20px", background: "white", border: "1px solid #ede9fe", boxShadow: "0 10px 30px rgba(76,29,149,.07)" },
  metricIcon: { width: "42px", height: "42px", borderRadius: "14px", display: "grid", placeItems: "center", color: "#6d28d9", background: "#f5f3ff" },
  ...
  searchIcon: { position: "absolute", left: "14px", top: "50%", transform: "translateY(-50%)", color: "#8b5cf6" },
  search: { width: "100%", boxSizing: "border-box", padding: "12px 14px 12px 40px", borderRadius: "13px", border: "1px solid #ddd6fe", outline: "none", fontSize: "13px" },
  select: { padding: "12px", borderRadius: "13px", border: "1px solid #ddd6fe", background: "white", color: "#4c1d95" },
  filterButton: { border: 0, borderRadius: "13px", padding: "12px 16px", background: "linear-gradient(135deg,#6d28d9,#8b5cf6)", color: "white", fontWeight: 800, cursor: "pointer", whiteSpace: "nowrap" },
  ...
  detailsButton: { border: "1px solid #ddd6fe", borderRadius: "10px", padding: "8px 10px", background: "white", color: "#6d28d9", fontWeight: 800, cursor: "pointer" },
  ...
  close: { width: "40px", height: "40px", borderRadius: "12px", border: "1px solid #e9e3f5", background: "white", color: "#6d28d9", cursor: "pointer" },
  ...
  manageCard: { marginTop: "18px", padding: "18px", borderRadius: "20px", background: "linear-gradient(135deg,#faf7ff,#fff)", border: "1px solid #e9ddff", display: "grid", gap: "14px" },
  manageActions: { display: "flex", flexWrap: "wrap", gap: "9px", alignItems: "center" },
  actionPrimary: { border: 0, borderRadius: "12px", padding: "11px 15px", background: "linear-gradient(135deg,#6d28d9,#8b5cf6)", color: "white", fontWeight: 850, cursor: "pointer" },
  actionSuccess: { border: 0, borderRadius: "12px", padding: "11px 15px", background: "#047857", color: "white", fontWeight: 850, cursor: "pointer" },
  actionWarn: { border: 0, borderRadius: "12px", padding: "11px 15px", background: "#b45309", color: "white", fontWeight: 850, cursor: "pointer" },
  actionDanger: { border: 0, borderRadius: "12px", padding: "11px 15px", background: "#b91c1c", color: "white", fontWeight: 850, cursor: "pointer" },
