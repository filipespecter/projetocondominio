...
class PlatformAuditRepository {
  ...
  async findPaginated(filters, page, limit) {
    ...
    try {
      ...
      // Fallback defensivo: a auditoria nunca derruba a Central por uma relação
      // histórica inconsistente. Os campos escalares continuam disponíveis.
      const [items, total] = await Promise.all([
        prisma.auditLog.findMany({ where, orderBy: { createdAt: "desc" }, skip, take: limit }),
        prisma.auditLog.count({ where }),
      ]);
      return { items: items.map((item) => ({ ...item, condominium: null, user: null, supportSession: null })), total };
    }
  }
