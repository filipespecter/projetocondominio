...
class PlatformCondominiumController {
  ...
  async changePlan(req, res, next) {
    try {
      const requestContext = { requestId: req.requestId ?? null, ipAddress: req.ip ?? null, userAgent: req.headers["user-agent"] ?? null };
      const result = await PlatformCondominiumService.changeClientPlan(req.params.id, req.body, req.user, requestContext);
      return res.status(200).json({ success: true, message: "Plano do cliente atualizado com sucesso.", data: result });
  ...
  async lifecycle(req, res, next) {
    try {
      const requestContext = { requestId: req.requestId ?? null, ipAddress: req.ip ?? null, userAgent: req.headers["user-agent"] ?? null };
      const result = await PlatformCondominiumService.changeClientStatus(req.params.id, req.body, req.user, requestContext);
      return res.status(200).json({ success: true, message: "Status do cliente atualizado com sucesso.", data: result });
    } catch (error) { return next(error); }
  }
