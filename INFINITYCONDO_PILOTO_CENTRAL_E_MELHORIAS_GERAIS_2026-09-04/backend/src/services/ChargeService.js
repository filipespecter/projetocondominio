...
class ChargeService {
  ...
  async findAllPlatform(query = {}) {
    const parseDate = (value, end = false) => {
      ...
      if (Number.isNaN(date.getTime())) throw new ApiError("Data financeira inválida.", 400);
      if (/^\d{4}-\d{2}-\d{2}$/.test(String(value))) date.setHours(end ? 23 : 0, end ? 59 : 0, end ? 59 : 0, end ? 999 : 0);
      return date;
    };
