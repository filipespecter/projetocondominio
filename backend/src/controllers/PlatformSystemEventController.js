...
class PlatformSystemEventController {
  ...
  async resolve(req, res, next) { try { return res.status(200).json({ success: true, message: "Evento marcado como resolvido.", data: await SystemEventService.resolve(req.params.id, req.user, req.body) }); } catch (e) { return next(e); } }
