...
class ReservationService extends BaseService {
  ...
  /**
   * Administração cria uma reserva em nome de
   * um morador real do condomínio.
   */
  async createAdministrative(
    ...
    await AuditLogService.logCreate({
      ...
      afterData:
        reservation,
      details:
        "Reserva criada pela administração em nome de morador.",
      requestContext,
    });
    ...
    if (
      ...
      reservation =
        await this.approve(
          ...
          condominiumId,
          authenticatedUser,
          "Reserva criada e aprovada pela administração.",
          requestContext
        );
    }
  ...
  async approve(
    ...
    if (!authenticatedUser?.id) {
      throw new ApiError(
        "Usuário responsável pela análise não identificado.",
        401
      );
    }
  ...
  /**
   * Reabre uma reserva rejeitada/cancelada.
   * Apenas a administração chama esta regra.
   */
  async reopen(
    id,
    condominiumId,
    ...
    if (
      ...
      throw new ApiError(
        "Somente reservas rejeitadas ou canceladas podem ser reabertas.",
        409
      );
    }
    ...
    if (!reservation) {
      throw new ApiError(
        "Não foi possível reabrir a reserva.",
        409
      );
    }
    ...
    await AuditLogService
      .logStatusChange({
        ...
        newStatus:
          "PENDING",
        details:
          "Reserva reaberta pela administração.",
        requestContext,
      });
