...
class ResidentService extends BaseService {
  ...
  /**
   * Sincroniza o status do apartamento conforme
   * a quantidade de moradores ativos vinculados.
   *
   * Regras:
   * - com morador ativo: OCCUPIED;
   * - sem morador ativo: VACANT;
   * - MAINTENANCE e INACTIVE não são alterados
   *   automaticamente por este método.
   */
  async syncApartmentOccupancy(
    apartmentId,
    condominiumId
  ) {
  ...
  async create(
    ...
    try {
      ...
      await this.syncApartmentOccupancy(
        data.apartmentId,
        condominiumId
      );
  ...
  async update(
    ...
    if (
      ...
      if (!residentUpdated) {
        throw new ApiError(
          "Morador não encontrado ou não pôde ser atualizado.",
          404
        );
      }
    }
  ...
  async changeApartment(
    ...
    if (!updated) {
      throw new ApiError(
        "Morador não encontrado ou não pôde ser transferido.",
        404
      );
    }
  ...
  /**
   * Diretório operacional de moradores.
   * O condominiumId vem do token autenticado.
   */
