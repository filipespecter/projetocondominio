...
function Prestadores() {
  ...
  async function excluirPrestador(id){if(!window.confirm("Deseja excluir este cadastro?"))return; try{await providerAccessApi.remove(id); await carregar();}catch(e){alert(e?.message??"Só acessos finalizados/cancelados podem ser excluídos.");}}
  ...
  async function excluirOperacional(id){if(!window.confirm("Deseja excluir este registro operacional?"))return; try{await operationalRecordApi.remove(id);await carregar();}catch(e){alert(e?.message??"Não foi possível excluir o registro.");}}
