...
export async function carregarDadosBI() {
  ...
  const visitantesTodos = valor(3);
  const encomendasTodas = valor(4);
  const ocorrenciasTodas = valor(10);
  const statusFinalVisitante = new Set(["LEFT", "DENIED", "CANCELED", "EXPIRED", "SAIU", "NEGADO", "CANCELADO"]);
  const statusFinalEncomenda = new Set(["DELIVERED", "CANCELED", "ENTREGUE", "CANCELADO"]);
  const statusFinalOcorrencia = new Set(["RESOLVED", "CLOSED", "CANCELED", "RESOLVIDO", "FECHADO", "CANCELADO"]);

  cacheDados.visitantes = visitantesTodos.filter((x) => !statusFinalVisitante.has(String(x.status || "").toUpperCase()));
  cacheDados.visitantes_historico = visitantesTodos.filter((x) => statusFinalVisitante.has(String(x.status || "").toUpperCase()));
  cacheDados.encomendas = encomendasTodas.filter((x) => !statusFinalEncomenda.has(String(x.status || "").toUpperCase()));
  cacheDados.encomendas_historico = encomendasTodas.filter((x) => statusFinalEncomenda.has(String(x.status || "").toUpperCase()));
  cacheDados.encomendas_esperadas = encomendasTodas.filter((x) => String(x.status || "").toUpperCase() === "EXPECTED");
  cacheDados.reservas = valor(5);
  cacheDados.condominio_prestadores = valor(6);
  cacheDados.prestadores_particulares_v2 = valor(6).filter((x) => Boolean(x.apartmentId));
  cacheDados.areasComuns = valor(7);
  cacheDados.avisos = valor(8);
  cacheDados.avisos_sindico = valor(8).filter((x) => String(x.audience || x.target || "").toUpperCase().includes("ADMIN"));
  cacheDados.notificacoesMorador = valor(9);
  cacheDados.notificacoes = valor(9);
  cacheDados.ocorrencias = ocorrenciasTodas.filter((x) => !statusFinalOcorrencia.has(String(x.status || "").toUpperCase()));
  cacheDados.historico_ocorrencias = ocorrenciasTodas.filter((x) => statusFinalOcorrencia.has(String(x.status || "").toUpperCase()));
  cacheDados.livro_ocorrencias = ocorrenciasTodas.filter((x) => String(x.type || "").toUpperCase() === "OCCURRENCE");
  cacheDados.sugestoesMorador = ocorrenciasTodas.filter((x) => ["SUGGESTION","COMPLAINT","REQUEST"].includes(String(x.type || x.category || "").toUpperCase()));
  cacheDados.sugestoes_reclamacoes = ocorrenciasTodas.filter((x) => ["SUGGESTION","COMPLAINT"].includes(String(x.type || "").toUpperCase()));
  cacheDados.operacional_condominio_v2 = valor(11);
  cacheDados.relatorios_operacionais = valor(11);
